...
contract StratMaticSushi is Ownable, ReentrancyGuard {
    ...
    function setAllowedBalance() private {
        IERC20(USDT).safeApprove(curRouter, uint256(-1));
        IERC20(USDC).safeApprove(curRouter, uint256(-1));
        IERC20(SUSHI).safeApprove(curRouter, uint256(-1));
        IERC20(WMATIC).safeApprove(curRouter, uint256(-1));
        address pairAddr = ICommonFactory(curFactory).getPair(USDT, USDC);
        IERC20(pairAddr).safeApprove(curRouter, uint256(-1));
    }
    ...
    // 此方法即复投，将本合约中的USDC-USDT LP复投到MDEX空投合约中进行挖矿
    // 可以由任意账号调用
