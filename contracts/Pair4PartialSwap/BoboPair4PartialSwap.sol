...
contract BoboPair is MixinAuthorizable, OrderStore, ReentrancyGuard {
    ...
    function swap(address _boboRouter, NFTInfo storage _orderInfo, 
        ...
        if(_orderInfo.inAmount == _amountIn) {   // 表示本订单的所有挂单金额都被吃完了
            ...
            uint256 childOrderId = addOrder(orderId, _orderInfo._bBuyQuoteToken, _orderInfo._spotPrice, _amountIn, minOutAmount);
    ...
    // taker
    function executeSpecifiedOrder(address _boboRouter, uint256 _orderId, uint256 _amountIn) public nonReentrant {  
        require(exManager.routerWhiteList(_boboRouter), "BoboPair: router NOT in whitelist!");                 
